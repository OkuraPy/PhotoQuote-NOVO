-- Phase C: deposit defined by the user (configurable) + a generic US contract template (all 50 states).
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS default_deposit_percent int;
ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS deposit_percent int;

-- createAgreement picks the template by `is_default` (fallback). Make the new generic US template
-- the default; keep the existing Florida one as a non-default, state-specific variant.
...
VALUES (
  'US', true, 'Service Agreement (United States)',
...
<ul>
<li>{{deposit_percent}}% deposit due upon signing of this Agreement: <strong>${{deposit_amount}}</strong></li>
<li>Remaining balance due upon completion of the work: <strong>${{balance_amount}}</strong></li>
</ul>
